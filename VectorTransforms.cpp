...
vector<vector<float>> VectorTransforms::Multiply(vector<vector<float>>& firstVector, vector<vector<float>>& secondVector)
{
...
void VectorTransforms::RotateAroundPoint(vector<vector<float>>& vectorToRotate, vector<float>& point, float angle)
{
...
bool VectorTransforms::bIsMatrix(vector<vector<float>>& vectorToCheck)
{
...
float VectorTransforms::degreesToRadians(float degrees)
{
